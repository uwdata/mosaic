...
`preagg` requires a supporting server, such as the [Go server with preaggregation enabled](../duckdb/go-server). SELECT and `preagg` failures preserve structured error codes and missing-table references as `ConnectorError` instances. Responses follow request order, so a build delays later requests on the same socket.
