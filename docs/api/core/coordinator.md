...
* _indexes_: Indexer options object. The _enabled_ flag (default `true`) determines if indexes should be used when possible. The _schema_ option (default `'mosaic'`) indicates the database schema in which materialized views for the pre-aggregates (indexes) should be created.
