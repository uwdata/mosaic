...
export default vg.plot(
  ...
  vg.text(
    vg.from("labels"),
    { x: "Date", y: vg.sql`Close / (SELECT MAX(Close) FROM stocks WHERE Symbol = source.Symbol AND Date = ${$point})`, dx: 2, text: "Symbol", fill: "Symbol", textAnchor: "start" }
  ),
  vg.lineY(
    vg.from("stocks"),
    { x: "Date", y: vg.sql`Close / (SELECT MAX(Close) FROM stocks WHERE Symbol = source.Symbol AND Date = ${$point})`, stroke: "Symbol" }
  ),
