...
import { queryEvents, clientEvents } from '@uwdata/mosaic-core';
...
/**
 * Return (and lazily create) the TanStack event clients.
 */
function createEventClients() {
  if (!_queryEventClient || !_clientEventClient) {
    _queryEventClient = new EventClient<QueryEventTypes>({
      pluginId: 'queries',
      debug: true
    });
    _clientEventClient = new EventClient<ClientEventTypes>({
      pluginId: 'clients',
      debug: true
    });
  }
  return {
    queryEventClient: _queryEventClient,
    clientEventClient: _clientEventClient
  };
}

/**
 * Bridge function that connects core AsyncDispatch events to TanStack EventClient.
 * This subscribes to core events and forwards them to TanStack devtools.
 */
function bridgeEvents() {
  const { queryEventClient, clientEventClient } = createEventClients();

  // Bridge query start events
  queryEvents.addEventListener('start', (payload) => {
    queryEventClient.emit('start', payload);
  });

  // Bridge query end events
  queryEvents.addEventListener('end', (payload) => {
    queryEventClient.emit('end', payload);
  });

  // Bridge client state events
  clientEvents.addEventListener('state', (payload) => {
    clientEventClient.emit('state', payload);
  });
}
...
export function initializeDevtools(document: Document) {
  // Bridge core events to TanStack EventClient
  bridgeEvents();
        ...
        render: (el: HTMLElement) => {
          ...
          const makeItem = (kind: string, payload: any) => {
            ...
            item.style.background =
              kind === 'start' ? 'rgba(0,0,255,0.02)' : 'rgba(0,128,0,0.02)';
            ...
            meta.textContent = `${
              payload.clientId ?? 'unknown client'
            ...
            if (kind === 'end') {
              const rows =
                payload.numRows != null ? String(payload.numRows) : '?';
              const latency =
              ...
              right.textContent = payload.materialized
                ? 'materialized'
                : 'not materialized';
            }
            ...
            info.textContent = `Client: ${
              payload.clientId ?? '—'
            } • Materialized: ${
              payload.materialized ? 'yes' : 'no'
            ...
            if (kind === 'end') {
              ...
              stats.textContent = `Latency: ${
                payload.latencyMs != null ? payload.latencyMs + ' ms' : '?'
              } • Rows: ${payload.numRows ?? '?'} • Bytes: ${
                payload.numBytes != null ? formatBytes(payload.numBytes) : '?'
              }`;
              details.appendChild(stats);

              // exemplarRows preview (if present)
              if (
                Array.isArray(payload.exemplarRows) &&
                payload.exemplarRows.length > 0
              ) {
      ...
      },
      {
        ...
        render: (el: HTMLElement) => {
          ...
          const renderClient = (
            ...
            right.textContent = `Latency: ${
              c.lastLatencyMs ?? '?'
            } ms • Rows: ${c.lastNumRows ?? '?'} • Bytes: ${
              c.lastNumBytes != null ? formatBytes(c.lastNumBytes) : '?'
            }`;
          ...
          const renderState = (state: ClientEventTypes['clients:state']) => {
            ...
            header.textContent = `Connected: ${
              state.numConnectedClients
            } — filter groups: ${state.coordinatorState?.numFilterGroups ?? 0}`;
