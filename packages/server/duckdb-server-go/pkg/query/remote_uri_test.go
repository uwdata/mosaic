...
func TestDBRemoteURILiteralRejection(t *testing.T) {
	...
	data, _, err := db.QueryArrow(t.Context(), "SELECT * FROM read_csv("+quoteLiteral(path)+")", nil, false)
	...
	data, _, err = db.QueryArrow(t.Context(), "SELECT * FROM read_csv("+list+") ORDER BY value", nil, false)
	require.NoError(t, err)
	assert.Equal(t, []map[string]any{{"value": float64(42)}, {"value": float64(43)}}, arrowRows(t, data))

	data, _, err = db.QueryArrow(t.Context(), "SELECT 'https://example.com' AS url WHERE url = 'https://example.com'", nil, false)
	require.NoError(t, err)
	assert.Equal(t, []map[string]any{{"url": "https://example.com"}}, arrowRows(t, data))

	_, _, err = db.QueryArrow(t.Context(), "SELECT * FROM read_csv('https://example.com/file.csv')", nil, false)
	require.ErrorIs(t, err, ErrAccessDenied)
	assert.ErrorContains(t, err, "remote URI prefix 'https://' is not allowed in path argument to function 'read_csv'")

	_, _, err = db.QueryArrow(t.Context(), "SELECT * FROM query('SELECT 42')", nil, false)
	require.ErrorIs(t, err, ErrAccessDenied)
	assert.ErrorContains(t, err, "nested SQL executor 'query' is not allowed")

	_, _, err = db.QueryArrow(
		t.Context(),
		"SELECT json_serialize_plan('SELECT * FROM read_csv(''https://example.com/file.csv'')')",
		nil,
		false,
	...
	_, _, err = db.QueryArrow(t.Context(), "PRAGMA import_database('s3://bucket/export')", nil, false)
