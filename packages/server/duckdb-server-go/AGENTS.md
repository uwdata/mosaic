...
- Update the catalog exemptions, reviewed macros, and collision allowlists in `builtin_functions_test.go` only when the matching DuckDB source justifies the exception. Verify parser-generated operators and syntax helpers with `json_serialize_sql` and executable SQL because some have no catalog row.
