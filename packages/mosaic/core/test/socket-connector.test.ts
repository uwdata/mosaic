...
describe('SocketConnector', () => {
  ...
  it('matches mixed preagg, Arrow, and exec responses in order', async () => {
    ...
    const arrow = connector.query({ type: 'arrow', sql: 'SELECT 2 AS x' });
    const last = exec('SELECT 3');
    socket().emit('open');
    expect(socket().sent.map(s => JSON.parse(s).type)).toEqual(['preagg', 'arrow', 'exec']);

    const response = { reference: { catalog: 'memory', schema: ['s'], table: 't' }, createdAt: '2026-09-11T00:00:00Z' };
  ...
  it.each([
    ...
    socket().emit('message', { data: JSON.stringify({
      error: 'missing table', code: 'table_not_found', reference: { catalog: 'memory', schema: ['s'], table: 't' }
    }) });
    const error = await result;
    expect(error).toBeInstanceOf(ConnectorError);
    expect(error).toMatchObject({ message: 'missing table', code: 'table_not_found', reference: { catalog: 'memory', schema: ['s'], table: 't' } });
  ...
  it.each([
    ...
    { name: 'JSON Arrow response', query: (c: SocketConnector) => c.query({ type: 'arrow', sql: 'SELECT 1' }), data: '{}' }
