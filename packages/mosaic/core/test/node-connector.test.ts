...
import { clausePoint, decodeIPC } from '../src/index.js';
...
describe('NodeConnector preagg', () => {
  it('materializes a temporary table and reuses it for identical SQL', async () => {
    ...
    const table = new TableRefNode([first.reference.catalog, ...first.reference.schema, first.reference.table]);
    const result = decodeIPC(await connector.query({ type: 'arrow', sql: `SELECT * FROM ${table} ORDER BY x` }));
    expect(result.toArray()).toEqual([{ x: 1, n: 1 }, { x: 2, n: 2 }]);
    expect(decodeIPC(await connector.query({ type: 'arrow', sql: 'SELECT count(*) AS n FROM duckdb_tables() WHERE temporary' })))
