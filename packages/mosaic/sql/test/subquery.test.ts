...
describe('Scalar subqueries', async () => {
  it('are supported', async () => {
    ...
    await expect(`SELECT * FROM ${subq} AS "t"`).toBeValidQuery(`SELECT * FROM (SELECT * FROM "t1") AS "t"`);
  });
  it('can be tested for inclusion', async () => {
    ...
    await expect(`SELECT ${test} FROM "t1"`).toBeValidQuery(`SELECT ("num1" IN (SELECT "num1" FROM "t1" LIMIT 3)) FROM "t1"`);
  });
});
