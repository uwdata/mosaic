...
describe('ClickHouse set operation overrides', () => {
  ...
  it.each([
    ['union', 'UNION DISTINCT'], ['unionAll', 'UNION ALL'],
    ['intersect', 'INTERSECT'], ['intersectAll', 'INTERSECT ALL'],
    ['except', 'EXCEPT'], ['exceptAll', 'EXCEPT ALL']
  ] as const)('scopes global clauses around %s', (method, op) => {
    const query = Query[method](
      Query.select('*').from('a').orderby('x').limit(2),
      Query.select('*').from('b')
    ).orderby('x').limit(3).offset(1);
    const original = query.toString();
    expect(query.toString(gen)).toBe(
      'SELECT * FROM ((SELECT * FROM "a" ORDER BY "x" LIMIT 2) '
      + `${op} (SELECT * FROM "b")) ORDER BY "x" LIMIT 3 OFFSET 1`
    );
    expect(query.toString()).toBe(original);
  });

  it('wraps shared CTEs even without global clauses', () => {
    const query = Query.with({ t: Query.select('x').from('data') }).unionAll(
      Query.select('x').from('t'),
      Query.select('x').from('t')
    );
    expect(query.toString(gen)).toBe(
      'WITH "t" AS (SELECT "x" FROM "data") '
      + 'SELECT * FROM ((SELECT "x" FROM "t") UNION ALL (SELECT "x" FROM "t"))'
    );
  });

  it('rejects percentage limits on set operations', () => {
    expect(() => Query.unionAll(
      Query.select('x').from('a'),
      Query.select('x').from('b')
    ).limitPercent(10).toString(gen)).toThrow(/LIMIT PERCENT is not supported/);
  });
