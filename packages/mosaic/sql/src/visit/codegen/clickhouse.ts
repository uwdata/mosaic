...
export class ClickHouseCodeGenerator extends SQLDialectCodeGenerator {
  ...
  visitSetOperation(node: SetOperation): string {
    ...
    const op = upper === 'UNION' ? 'UNION DISTINCT' : node.op;
    const body = node.queries
      .map(query => `(${this.toString(query)})`)
      .join(` ${op} `);
    // ClickHouse applies trailing clauses to the last operand, not the set result.
    const wrap = node._with.length || node._orderby.length
      || node._limit != null || node._offset != null;
    return this.visitQuery(node, [wrap ? `SELECT * FROM (${body})` : body]);
  }
...
export const clickHouseCodeGenerator = /* @__PURE__ */ new ClickHouseCodeGenerator();
