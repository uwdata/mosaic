...
describe("Query", () => {
  it("selects column name strings", () => {
    ...
    assert.strictEqual(
      Query.select("foo", "bar", "baz").from("data").toString(),
      query
    );

    assert.strictEqual(
      Query.select("foo", "bar", "baz").from(relation("data")).toString(),
      query
    );
    ...
    assert.strictEqual(
      Query.select("foo").select("bar").select("baz").from("data").toString(),
      query
    );
  });
  ...
  it("selects distinct columns", () => {
    assert.strictEqual(
      Query.select("foo", "bar", "baz").distinct().from("data").toString(),
  ...
  it("selects aggregates", () => {
    ...
    assert.strictEqual(
      Query.select({ min: min("foo"), max: max("foo") })
        .from("data")
        .toString(),
    ...
    assert.strictEqual(
      Query.select({ min: min("foo").where(gt("bar", 5)) })
        .from("data")
        .toString(),
  ...
  it("selects grouped aggregates", () => {
    ...
    assert.strictEqual(
      Query.select({ min: min("foo"), max: max("foo"), bar: "bar", baz: "baz" })
        .from("data")
        .groupby("bar", "baz")
        .toString(),
      query
    ...
    assert.strictEqual(
      Query.select({ min: min(foo), max: max(foo), bar, baz })
        .from("data")
        .groupby([bar, baz])
        .toString(),
      query
  ...
  it("selects ordered rows", () => {
    ...
    assert.strictEqual(
      Query.select("*").from("data").orderby(bar, desc(baz)).toString(),
      query
    );
    ...
    assert.strictEqual(
      Query.select("*").from("data").orderby(bar).orderby(desc(baz)).toString(),
      query
    );
  ...
  it("selects sampled rows", () => {
    assert.strictEqual(
      Query.select("*").from("data").sample(10).toString(),
      'SELECT * FROM "data" USING SAMPLE 10 ROWS'
    );

    assert.strictEqual(
      Query.select("*").from("data").sample({ rows: 10 }).toString(),
      'SELECT * FROM "data" USING SAMPLE 10 ROWS'
    );

    assert.strictEqual(
      Query.select("*").from("data").sample(0.3).toString(),
      'SELECT * FROM "data" USING SAMPLE 30 PERCENT'
    );

    assert.strictEqual(
      Query.select("*").from("data").sample({ perc: 30 }).toString(),
      'SELECT * FROM "data" USING SAMPLE 30 PERCENT'
    );
  ...
  it("selects over windows", () => {
    assert.strictEqual(
      Query.select({ lead: sql`lead("foo") OVER "win"` })
        .from("data")
  ...
  it("selects with common table expressions", () => {
    assert.strictEqual(
      Query.with({ a: Query.select("*").from("data") })
        .select("foo", "bar")
        .from("a")
        .toString(),
