...
describe("Binary operators", () => {
  ...
  it("include IS DISTINCT FROM comparisons", () => {
    assert.strictEqual(
      String(isDistinct(column("foo"), null)),
      '("foo" IS DISTINCT FROM NULL)'
    );
    assert.strictEqual(
      String(isDistinct("foo", null)),
      '("foo" IS DISTINCT FROM NULL)'
    );
  });
  it("include IS NOT DISTINCT FROM comparisons", () => {
    assert.strictEqual(
      String(isNotDistinct(column("foo"), null)),
      '("foo" IS NOT DISTINCT FROM NULL)'
    );
    assert.strictEqual(
      String(isNotDistinct("foo", null)),
      '("foo" IS NOT DISTINCT FROM NULL)'
    );
  });
});
...
describe("Range operators", () => {
  ...
  it("test not within exclusive ranges", () => {
    ...
    assert.strictEqual(
      String(isNotBetween("a", [0, 1], true)),
      'NOT (0 <= "a" AND "a" < 1)'
    );
  });
});
