    ...
    def to_dict(self, keep_none_values: bool):
        ...
        for field, value in vars(self).items():
            ...
            if isinstance(value, dict):
                val_as_dict = {
                    revert_validation(k): _todict(v)
                    for k, v in value.items()
                    if keep_none_values or v is not None
                }
            elif isinstance(value, list):
                val_as_dict = [
                    _todict(i) for i in value if keep_none_values or i is not None
                ]
        ...
        if not class_dictionary:
            ...
            and list(class_dictionary.keys())[0] == "value"
        ):
