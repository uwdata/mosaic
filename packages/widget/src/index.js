 ...
 * @property {string} index_schema The database schema in which to store
 *  materialized views (default 'mosaic').
...
export default {
  ...
  initialize(view) {
    view.model.set('index_schema', coordinator().indexer.schema);
  },
  ...
  render(view) {
    ...
    const getIndexSchema = () => view.model.get('index_schema');
    ...
    function configureCoordinator() {
      coordinator().indexer.schema = getIndexSchema();
    }

    view.model.on('change:index_schema', () => configureCoordinator());
